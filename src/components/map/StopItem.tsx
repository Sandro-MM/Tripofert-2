...
const StopItem = ({ position, label, onAdd, onRemove, description, isSelected, image, time }) => {
    ...
    return (
        <div className={`relative w-full my-[40px] max-w-[1080px] h-[430px] rounded-[1.5rem] mx-auto ${isSelected ? 'selected-marker' : ''}`}>
            ...
            <div className={`absolute top-0 left-0 w-1/4 min-w-[200px] h-full rounded-[1.5rem] sm:backdrop-blur-sm ${isSelected ? 'sm:bg-buttons/70 bg-buttons/50' : 'sm:bg-bg/70 bg-bg/50'}`}>
                ...
                <div className="flex items-center justify-center gap-3 absolute bottom-4 left-0 w-full">
